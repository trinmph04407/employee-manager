...
<body>
	<div class="container" >
		<s:form theme="bootstrap" cssClass="well form-vertical" action="createEmployeeSucces" enctype="multipart/form-data">
			<h1 align="center">
				<s:text name="label.employee.title.create" />
			</h1>
			
			<br>
			<div style="padding:0 350px 0 350px">
			<s:hidden name="id" />
			<s:textfield key="form.code" />
			<s:textfield  key="form.nameStaff" />
			<s:select list="#{'false':'Nu', 'true':'Nam'}"
			key="form.gender">
			</s:select>
			<s:textfield  key="form.birthday" type="date"/>
			<s:file  key="photo" />
